...
joints_human_list = [    'jT1C7_roty', #0
                         'jT9T8_roty', #1
                         'jL1T12_roty',#2
                         'jL4L3_roty', #3
                         'jL5S1_roty', #4
...
# length_initial = np.ones(len(length_initial))
...
solver = cs.Opti()
p_opts = {}
s_opts = {'linear_solver':'mumps','hessian_approximation':'limited-memory'}
...
OptimizerHelper = Optimizer(hardwareOptimizationPartList, urdf_path, joints_ctrl_name_list, root_link,contact_frames_robot, number_of_points, add_denstiy, consider_hardware_parameters)
...
# defining optimization problem tatau[i]u[i]
...
# print(key_word)
# for i in range(number_of_points):
#     print("#### Reference height ####", height_hands_start[i])
#     for number_joint in range(5): 
#         print("Human Torques ",joints_human_list[number_joint], " ", tau[OptimizerHelper.N_DoF+number_joint,i])

# # print(OptimizerHelper.s_opt_all[:,:3])
# # for number_joint in range(5): 
#     # norm_joint = np.linalg.norm(tau[OptimizerHelper.N_DoF+number_joint,:2])
#     # print("Norm for Joint ", joints_human_list[number_joint] , " ", norm_joint)
